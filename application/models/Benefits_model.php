...
class benefits_model extends CI_Model
{
 ...
 function getproductssById($id)
  {
    $this->db->select('id,b_id,c_image');
    $this->db->from('tbl_benefit_img');
  ...
  {
    foreach ($data as $d) {
      $this->db->insert("tbl_benefit_img", $d);
    }
  }
  ...
  function deletebrands($bId, $userInfo)
    {
        ...
        $this->db->update('tbl_benefits', $userInfo);
  ...
  function getbenefitsById($id)
  {
    ...
    $this->db->select('tbl_benefits.*,tbl_course_category.cat_name,tbl_course_subcategory.subcat_name,tbl_course_subcategory2.subcat2_name,tbl_designation.designation')
      ->from('tbl_benefits')

 ...
 */
class Conversation_Optimaization extends BaseController
{
	/**
	...
	{
		parent::__construct();
		$this->load->model('Stplacement_model');
	...
	public function index()
	{
		...
		if ($this->isAdmin() == TRUE) {
			...
			$this->global['pageTitle'] = 'Deskdigi  : Conversation_Optimaization';
			$this->loadViews("admin/gallery/listGallery", $this->global,  NULL);
		}
	}
	...
	function edit($id = NULL)
	{
		...
		if ($this->isAdmin() == TRUE || $id == 1) {
			...
			$data['allData'] = $this->Stplacement_model->getplacement();

			$this->global['pageTitle'] = 'Deskdigi : Conversation_Optimaization';

			$this->loadViews("admin/student_placement/editplace", $this->global, $data, NULL);
		}
	}
	...
	function editstudentplacement()
	{
		...
		if ($this->input->post('submit') !== NULL) {
			...
			if (!empty($_FILES["slid_img1"]['name'])) {
				...
				$slid_img1 = $this->db->get_where('tbl_stplacement', array('id' => 1))->row_array()['slid_img1'];
			}
			if (!empty($_FILES["slid_img2"]['name'])) {
				...
				$slid_img2 = $this->db->get_where('tbl_stplacement', array('id' => 1))->row_array()['slid_img2'];
			}
			// upload sec1_image 
			if (!empty($_FILES["pi1"]['name'])) {
				...
				$pi1 = $this->db->get_where('tbl_stplacement', array('id' => 1))->row_array()['pi1'];
			}
			...
			if (!empty($_FILES["pi2"]['name'])) {
				...
				$pi2 = $this->db->get_where('tbl_stplacement', array('id' => 1))->row_array()['pi2'];
			}
			...
			if (!empty($_FILES["pi3"]['name'])) {
				...
				$pi3 = $this->db->get_where('tbl_stplacement', array('id' => 1))->row_array()['pi3'];
			}
			...
			if (!empty($_FILES["pi4"]['name'])) {
				...
				$pi4 = $this->db->get_where('tbl_stplacement', array('id' => 1))->row_array()['pi4'];
			}
			...
			if (!empty($_FILES["pi5"]['name'])) {
				...
				$pi5 = $this->db->get_where('tbl_stplacement', array('id' => 1))->row_array()['pi5'];
			}
			...
			if (!empty($_FILES["pi6"]['name'])) {
				...
				$pi6 = $this->db->get_where('tbl_stplacement', array('id' => 1))->row_array()['pi6'];
			}
			...
			if (!empty($_FILES["pi7"]['name'])) {
				...
				$pi7 = $this->db->get_where('tbl_stplacement', array('id' => 1))->row_array()['pi7'];
			}
			...
			$result = $this->Stplacement_model->editplacement($homeInfo, 1);
			...
			redirect('admin/student_placement/edit');
		}
		redirect('admin/student_placement/edit');
	}
}

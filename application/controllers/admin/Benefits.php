 ...
 public function delete()
  ...
  public function viewbenefits()
  {
    ...
    if ($this->isAdmin() == TRUE) {
      ...
      $this->global['pageTitle'] = 'Fincore_Analytics :benefits';
  ...
  function editbenefits()
  {
    ...
    if ($this->isAdmin() == TRUE) {
      ...
      $data['training_dataa'] = $this->benefits_model->getproductssById($id);
      ...
      if (!empty($_FILES['companies']['name'])) {
        ...
        for ($i = 0; $i < $filesCount; $i++) {
          ...
          if ($this->upload->do_upload('file')) {
            ...
            $uploadData[$i]['date_created'] = date("Y-m-d H:i:s");
            $uploadData[$i]['status'] = "Active";
                   $uploadData[$i]['b_id'] = $this->input->post('id');}
            else {
            redirect(base_url() . 'admin/Benefits/viewbenefits');
          }
        }
        if (!empty($uploadData)) {
          // Insert files data into the database
          $insert = $this->benefits_model->updatess($uploadData);
...
function deletebenefits(){
        ...
        else
        {
          ...
          $result = $this->benefits_model->deletebrands($bId, $userInfo);
  ...
  public function datatable_json()
  {
    ...
    foreach ($records['data']  as $row) {
      ...
      $data[] = array(
        ...
        '<a title="Edit/View" class="update btn btn-primary" href="' . base_url() . 'admin/benefits/editbenefits/' . $row['id'] . '"> <i class="fa fa-pencil-square-o"></i></a>|<a class="btn btn-sm btn-danger deletebenefits" href="#" data-bid="' . $row['id'] . '" title="Delete"><i class="fa fa-trash"></i></a>',
      );
    }
